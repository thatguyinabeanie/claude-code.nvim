...
describe("Safe Window Toggle", function()
  ...
  describe("hide window without stopping process", function()
    it("should hide visible Claude Code window but keep process running", function()
      ...
      local config = {
        ...
        window = { position = "botright", start_in_normal_mode = false }
      }
      
      local git = {
        get_git_root = function() return "/test/project" end
      }
      ...
      vim.api.nvim_win_close = function(win, force)
        table.insert(closed_windows, {win = win, force = force})
      end
  ...
  describe("process state management", function()
    it("should maintain process state when window is hidden", function()
      ...
      vim.api.nvim_buf_is_valid = function(buf) return buf == bufnr end
      vim.fn.win_findbuf = function(buf) return {100} end  -- Visible
    ...
    it("should detect when hidden process has finished", function()
      ...
      vim.api.nvim_buf_is_valid = function(buf) return buf == bufnr end
      vim.fn.win_findbuf = function(buf) return {} end  -- Hidden
      
      -- Test: Show window of finished process
      terminal.safe_toggle(claude_code, {git = {multi_instance = true}}, {})
  ...
  describe("user notifications", function()
    it("should notify when hiding window with active process", function()
      ...
      local claude_code = {
        claude_code = {
          instances = { test = bufnr },
          process_states = {
            test = { status = "running", hidden = false }
          }
        }
      }
      
      vim.api.nvim_buf_is_valid = function() return true end
      vim.fn.win_findbuf = function() return {100} end
      vim.api.nvim_win_close = function() end
      
      -- Test: Hide window
      terminal.safe_toggle(claude_code, {git = {multi_instance = false}}, {})
    ...
    it("should notify when showing window with completed process", function()
      ...
      local claude_code = {
        claude_code = {
          instances = { test = bufnr },
          process_states = {
            test = { status = "finished", hidden = true }
          }
        }
      }
    ...
    it("should handle multiple hidden Claude instances independently", function()
      ...
      local claude_code = {
        claude_code = {
          ...
          process_states = {
            ["project1"] = { status = "running", hidden = true },
            ["project2"] = { status = "running", hidden = false }
          }
        }
      }
      ...
      vim.fn.win_findbuf = function(buf)
        if buf == project1_buf then return {} end  -- Hidden
        if buf == project2_buf then return {100} end  -- Visible
        return {}
      end
  ...
  describe("edge cases", function()
    it("should handle buffer deletion gracefully", function()
      ...
      local claude_code = {
        claude_code = {
          instances = { test = bufnr },
          process_states = { test = { status = "running" } }
        }
      }
      
      -- Mock deleted buffer
      vim.api.nvim_buf_is_valid = function(buf) return false end
